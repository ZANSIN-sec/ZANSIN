...
class CheckDocker(object):
    def __init__(self, utility, host="127.0.0.1", path="/", debug=False):
        self.utility = utility
        self.utility.target = host
        self.point = float(self.utility.point_dockerapi)
        self.name = 'CheckDocker'
        self.host = host
        self.path = path
        self.debug = debug
        self.description = "The module can check if the web application is currently working and also can check if the Docker API is currently open."
        self.url = self.utility.api_docker_url # ex: http://127.0.0.1/
    ...
    def test(self):
        ...
        if status:
            if self.debug:
                self.utility.print_message(self.utility.note, '%s: Docker API is currently open(target: %s)' % (self.name, self.url))
            return_values["description"] = "Docker API is currently open(Version: %s)" % (json.loads(res.text).get("Version"))
            return_values["result"] = False
            return return_values
        else:
            if self.debug:
                self.utility.print_message(self.utility.fail, '%s: Docker API is currently closed(target: %s)' % (self.name, self.url))
            return_values["description"] = "Docker API is currently closed."
